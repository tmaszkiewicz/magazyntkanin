...
def generuj_etykiete_tkaniny_podwojna(nazwa_tkaniny, barcode=[], L='', R='', data_dostawy='', M='', szerokosc=''):
    shift = 5.4
    c = canvas.Canvas("tmp/etykieta_podwojna.pdf", pagesize=(7.8 * cm, (5.4+shift) * cm))
    ilosc = len(barcode)
    for i, etykieta in enumerate(range(ilosc)):
        if len(nazwa_tkaniny) < 15:
            c.setFont('Courier-Bold', size=24)
            ...
            c.setFont('Courier-Bold', size=14)
        c.drawCentredString(3.9 * cm, (4.5+shift) * cm, nazwa_tkaniny)
        qr_code = qr.QrCodeWidget(barcode[i])
        bounds = qr_code.getBounds()
        width = bounds[2] - bounds[0]
        height = bounds[3] - bounds[1]
        d = Drawing(60, 60, transform=[60. / width, 0, 0, 60. / height, 0, 0])
        d.add(qr_code)
        renderPDF.draw(d, c, 2.8 * cm, (2+shift) * cm)
        c.setFont('Courier-Bold', size=10)
        c.drawString(3.5 * cm, (3.9+shift) * cm, barcode[i])
        c.setFont('Courier-Bold', size=18)
        c.drawString(0.2 * cm, (3.2+shift) * cm, 'L:')
        c.setFont('Courier-Bold', size=14)
        c.drawString(1.1 * cm, (3.2+shift) * cm, L)
        c.drawString(1.1 * cm, (3.2+shift) * cm, "_____")
        c.setFont('Courier-Bold', size=18)
        c.drawString(0.2 * cm, (1.7+shift) * cm, 'R:')
        c.setFont('Courier-Bold', size=14)
        c.drawString(1.1 * cm, (1.7+shift) * cm, R)
        c.drawString(1.1 * cm, (1.7+shift) * cm, "_____")
        c.setFont('Courier-Bold', size=18)
        c.drawString(4.8 * cm, (3.2+shift) * cm, 'M:')
        c.setFont('Courier-Bold', size=14)
        if not M == '':
            c.drawString(5.6 * cm, (3.2+shift) * cm, str(float(M)) + ' m')
        c.drawString(5.6 * cm, (3.2+shift) * cm, "_____")
        c.setFont('Courier-Bold', size=18)
        c.drawString(4.4 * cm, (1.7+shift) * cm, 'SZ:')
        c.setFont('Courier-Bold', size=14)
        if not szerokosc == '':
            c.drawString(5.6 * cm, (1.7+shift) * cm, str(szerokosc) + ' mm')
        c.drawString(5.6 * cm, (1.7+shift) * cm, "_____")
        c.setFont('Courier-Bold', size=18)
        c.drawString(1.87 * cm, (0.9+shift) * cm, 'Data:')
        c.setFont('Courier-Bold', size=14)
        if isinstance(data_dostawy, datetime):
            c.drawString(3.9 * cm, (0.9+shift) * cm, data_dostawy.strftime("%d/%m/%Y"))
        else:
            c.drawString(3.9 * cm, (0.9+shift) * cm, data_dostawy)
        ...
        d = Drawing(60, 60, transform=[60. / width, 0, 0, 60. / height, 0, 0])
        d.add(qr_code)
        renderPDF.draw(d, c, 2.8 * cm, 0 * cm)
